...
2. **[Catalog Service](file:///services/catalog)**: Manages product inventories and handles product search using a TF-IDF lexical search engine with hash-projected vectors and manual query expansion.
...
5. **[Payment Service](file:///services/payment)**: Implements a simulated payment flow with a documented extension point for real Stripe integration (controlled by `SIMULATE_PAYMENTS` flag).
...
* SQLAlchemy sessions set connection-level context (`app.current_tenant_id`) via `set_config()` at the start of each transaction.
