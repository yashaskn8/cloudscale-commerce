"""
Product Search Engine — TF-IDF Lexical Search with Hash-Projected Vectors.

Implements a lightweight product search pipeline using TF-IDF vectorization with
dimensionality-reduced dense embeddings via deterministic hash projection. Results
are cached in Redis for sub-millisecond retrieval on repeat queries.

This is NOT a neural/semantic search engine (no transformer model is used). It is
a well-implemented TF-IDF lexical search with manual query expansion that
demonstrates the correct architectural pattern (embed → index → search → cache).
...
Architecture:
  1. EmbeddingEngine: Generates dense float vectors from text using a vocabulary-
     ...
     projected into fixed dimensions via deterministic hashing.
  2. VectorIndex: In-memory brute-force index for cosine similarity search across
     product embeddings. In production, this would be replaced by pgvector or a
     dedicated vector DB (Pinecone, Qdrant, Weaviate).
  3. ProductSearchService: Orchestrates embedding generation, similarity search,
     caching, and Prometheus telemetry.

Upgrade path to real semantic search:
  - Swap EmbeddingEngine for sentence-transformers (e.g. all-MiniLM-L6-v2)
  - Replace VectorIndex with pgvector (CREATE EXTENSION vector;
    ALTER TABLE products ADD COLUMN embedding vector(384);)
  - The rest of the pipeline (caching, API, telemetry) remains unchanged.
"""
