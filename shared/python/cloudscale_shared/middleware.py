...
class TenantContextMiddleware(BaseHTTPMiddleware):
    ...
    Also sets the security.current_tenant_id ContextVar so that database sessions
    automatically call SET LOCAL for PostgreSQL RLS enforcement.
    """

    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        tenant_id = request.headers.get("X-Tenant-ID") or request.headers.get("x-tenant-id") or "default-tenant"

        # Bind tenant context for logging and legacy access
        token = tenant_id_context.set(tenant_id)
        structlog.contextvars.bind_contextvars(tenant_id=tenant_id)

        # Set security ContextVar for RLS database session wiring
        from cloudscale_shared.security import current_tenant_id

        security_token = current_tenant_id.set(tenant_id)
        ...
        try:
            ...
            current_tenant_id.reset(security_token)
